...
from libdpy.assignment_specific.privacy_auditing.lecture_figures import (
    ...
    plot_audit_confusion_matrix,
    plot_confidence_rectangle,
    plot_epsilon_histogram_naive_safe,
    plot_repeated_audit_points,
    plot_sampled_densities,
    plot_threshold_event,
)
...
```{python}
fig = plot_sampled_densities(samples_neg_roc, samples_pos_roc)
plt.show()
```
...
```{python}
fig = plot_threshold_event(samples_neg_roc, samples_pos_roc, tau_star)
plt.show()
```
...
```{python}
fig = plot_audit_confusion_matrix(audit_main)
plt.show()
```
...
![Empirical ROC accumulating over 200 samples.](../../../_generated/animations/lecture-presentations/privacy-auditing/empirical-roc.gif){width="70%" fig-align="center"}
...
fig = plot_repeated_audit_points(runs, reference=reference_point)
plt.show()
```
...
```{python}
fig = plot_confidence_rectangle(audit_main)
plt.show()
```
...
fig = plot_epsilon_histogram_naive_safe(plug_vals, safe_vals, reference_eps)
plt.show()
```
